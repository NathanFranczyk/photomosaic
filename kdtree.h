...
class KDTree
{
  public:
    ...
    void KDTreehelper(int target, int left, int right, int curdim);
    Point<Dim> select(int left, int right,int k, int curdim);
    /**
  ...
  private:
     ...
     */
     void neighborhelp(const Point<Dim>& startpoint, Point<Dim>& closest, int curdim, int size, int b, int& radius, bool& test) const;
     int finddistance(Point<Dim> end, Point<Dim> start) const;
};
